...
export default function DailyVideoScreen() {
  ...
  const generateMutation = useMutation({
    mutationFn: async () => {
      const response = await api.post(`/videos/group/${groupId}/daily`, null, { timeout: 180000 });
      return response.data as { status: string; output_url?: string };
    },
    onMutate: () => setGenerating(true),
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['daily-video', groupId, 'rolling'] });
    },
    onError: (err) => {
      const message =
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const localUri = `${FileSystem.documentDirectory}withlog-summary-${groupId}-recent-24h.mp4`;
  ...
  return (
    <SafeAreaView style={[styles.container, isDark && styles.containerDark]} edges={['bottom']}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <Text style={[styles.title, isDark && styles.titleDark]}>최근 24시간 요약 영상</Text>
        <Text style={[styles.sub, isDark && styles.subDark]}>지금부터 직전 24시간 안의 기록을 인원수에 맞춰 세로 스택으로 묶어 보여줍니다.</Text>
        <View style={[styles.infoCard, isDark && styles.infoCardDark]}>
          <Text style={styles.dateLabel}>대상 구간 최근 24시간</Text>
